...
type PlusOrMinus<S extends string> = S extends `${infer A}${infer R}`
  ? A extends "+" | "-"
    ? A
    : ""
  : "";
type Number<S extends string> = S extends `${infer A}${infer R}`
  ? A extends "+" | "-" | "%"
    ? Number<R>
    : `${A}${Number<R>}`
  : "";
type Unit<S extends string> = S extends `${infer A}${infer R}`
  ? A extends "%"
    ? A
    : Unit<R>
  : "";

type PercentageParser<A extends string> = [PlusOrMinus<A>, Number<A>, Unit<A>];
