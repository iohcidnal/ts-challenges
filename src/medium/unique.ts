...
type IsInResult<T extends any, R extends any[] = []> = R extends [
  infer F,
  ...infer Rest
]
...
type Unique<T extends any[], R extends any[] = []> = T extends [
  ...
  ? IsInResult<F, R> extends true
    ? Unique<Rest, R>
    : Unique<Rest, [...R, F]>
  : R;

// `T[number]` will dedup `1` and `number` or `"a"` and `string`
type TupleToUnion<T extends any[]> = T[number];
type Foo = TupleToUnion<[string, number, 1, "a", 1, string, 2, "b", 2, number]>;
//   ^?
